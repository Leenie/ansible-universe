...
def generate_readme(role):
	...
	template = """
		...
		|------|-------|-------------|
		{% for key in variables.keys()|sort %}| {{ "`" + key + "`" }} | {{ ("_" + variables[key].type + "_") if variables[key].type != None else "" }} {{ ("`" + variables[key].value|string + "`") if variables[key].value != None else "" }} | {{ variables[key].description or "" }} |
		{% endfor %}
		{% else %}
		No variable.
		{% endif %}
